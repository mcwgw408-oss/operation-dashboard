...
const expectedDir = resolve(currentDir, "expression-expected");
const shouldUpdate = process.argv.includes("--update");
const harness = createBrainGoldenHarness();
const failures = [];

for (const fixture of fixtures) {
  const actual = normalizeBrainExpressionGoldenResult(harness.runExpressionFixture(fixture));
...
if (failures.length) {
  console.error(`brain expression golden failed: ${failures.join(", ")}`);
  console.error("Run with --update only after intentionally accepting changed Brain expression outputs.");
  process.exit(1);
}

console.log("brain expression golden passed");
