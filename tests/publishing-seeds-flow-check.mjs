...
setPublishingSeedUsed(publishingSeeds[0], true);
const usedSeeds = JSON.parse(localStorage.getItem(PUBLISHING_SEEDS_STORAGE_KEY));
setPublishingSeedUsed(publishingSeeds[0], false);
const unusedSeeds = JSON.parse(localStorage.getItem(PUBLISHING_SEEDS_STORAGE_KEY));
...
check(result.usedSeeds[0].used === true && Boolean(result.usedSeeds[0].usedAt), "Seed used check was not persisted");
check(result.unusedSeeds[0].used === false && result.unusedSeeds[0].usedAt === "", "Seed used check was not cleared");
